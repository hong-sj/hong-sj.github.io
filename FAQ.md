...
If the website does not load the theme and its links or layout look broken, make sure to correctly specify the `url` and `baseurl` paths in `_config.yml`. Set `url` to `https://<your-github-username>.github.io` or to `https://<your.custom.domain>` if you are using a custom domain. If you are deploying a <ins>personal</ins> or <ins>organization</ins> website, leave `baseurl` **empty** (do **NOT** delete it). If you are deploying a project page, set `baseurl: /<your-project-name>/`. If all previous steps were correct, your browser may need to fetch the stylesheet again. For this, you can:
...
- `broken-links-site.yml`: checks local links in the generated site.
- `deploy.yml`: builds the site, checks generated HTML accessibility, and deploys to GitHub Pages.
- `prettier.yml`: verifies repository formatting.
- `render-cv.yml`: generates the RenderCV PDF.
- `test-scholar-sync.yml`: runs Scholar synchronization regression tests.
- `update-citations.yml` and `update-publications.yml`: refresh Scholar-derived data.
